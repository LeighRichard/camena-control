...
class VideoStreamService:
    ...
    def encode_jpeg(self, frame: np.ndarray) -> Optional[bytes]:
        ...
        优先使用 OpenCV（性能最佳），其次使用 PIL
        
        Args:
            frame: BGR 格式的图像帧
            
        Returns:
            JPEG 编码后的字节数据，编码失败返回 None
            
        Raises:
            RuntimeError: 当所有编码方式都不可用时抛出
        """
        if frame is None or frame.size == 0:
            logger.warning("尝试编码空帧")
            return None
        ...
        try:
            ...
            logger.warning(f"OpenCV JPEG 编码失败: {e}")
        ...
        try:
            ...
            logger.warning(f"PIL JPEG 编码失败: {e}")
        
        # 所有编码方式都不可用，记录错误并返回 None
        logger.error("无法编码 JPEG：OpenCV 和 PIL 都不可用")
        return None
