...
class OrbbecController(BaseCameraController):
    ...
    def initialize(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # 等待几帧让相机稳定（带重试机制）
            stable_frames = 0
            max_attempts = self.MIN_STABLE_FRAMES * 3  # 最多尝试3倍的帧数
            for attempt in range(max_attempts):
                try:
                    ...
                    logger.debug(f"等待稳定帧失败 (尝试 {attempt + 1}/{max_attempts}): {e}")
                    # 继续尝试
            
            if stable_frames < self.MIN_STABLE_FRAMES:
                logger.warning(f"仅获取到 {stable_frames}/{self.MIN_STABLE_FRAMES} 个稳定帧")
    ...
    # 帧获取相关常量
    MAX_FRAME_RETRY = 3  # 最大重试次数
    FRAME_TIMEOUT_MS = 1000  # 帧获取超时时间（毫秒）
    ...
    def capture(
        ...
        try:
            ...
            for _ in range(wait_frames):
                for retry in range(self.MAX_FRAME_RETRY):
                    try:
                        self._pipeline.wait_for_frames(timeout_ms=self.FRAME_TIMEOUT_MS)
                        frames_dropped += 1
                        break
                    except Exception as e:
                        if retry == self.MAX_FRAME_RETRY - 1:
                            logger.warning(f"丢弃帧超时: {e}")
                        else:
                            continue
            ...
            for retry in range(self.MAX_FRAME_RETRY):
                try:
                    frameset = self._pipeline.wait_for_frames(timeout_ms=self.FRAME_TIMEOUT_MS)
                    if frameset is not None:
                        break
                except Exception as e:
                    last_error = e
                    if retry < self.MAX_FRAME_RETRY - 1:
                        logger.debug(f"获取帧集重试 ({retry + 1}/{self.MAX_FRAME_RETRY}): {e}")
            
            if frameset is None:
                self._status = CameraStatus.READY
                error_msg = f"获取帧集失败: {last_error}" if last_error else "获取帧集失败"
            ...
            # 检查 pipeline 是否仍然有效
            try:
                # 尝试获取设备状态来验证 pipeline 是否正常
                if self._pipeline is not None:
                    self._pipeline.get_device_list()
            except Exception:
                # Pipeline 可能已损坏，标记为错误状态
                self._status = CameraStatus.ERROR
                self._last_error = str(e)
                logger.error(f"Pipeline 可能已损坏: {e}")
                return None, f"Pipeline 错误: {e}"
            
            # 恢复为就绪状态
            self._status = CameraStatus.READY
            
            # 检查是否是 OBException 类型
            if ORBBEC_AVAILABLE and hasattr(e, '__class__') and e.__class__.__name__ == 'OBException':
    ...
    def configure(self, config: CameraConfig) -> Tuple[bool, str]:
        ...
        # 参数验证
        if config is None:
            return False, "配置参数不能为 None"
        
        if not isinstance(config, CameraConfig):
            return False, f"配置参数类型错误，期望 CameraConfig，实际为 {type(config).__name__}"
        
        # 验证配置参数有效性
        is_valid, validation_error = config.validate()
        if not is_valid:
            return False, f"配置参数无效: {validation_error}"
