...
def _parse_config(data: Dict[str, Any]) -> SystemConfig:
    ...
    if 'camera' in data:
        ...
        if 'orbbec' in cam_data:
            orbbec_data = cam_data['orbbec']
            
            # 解析彩色流配置
            if 'color' in orbbec_data:
                color_data = orbbec_data['color']
                orbbec_config.color = OrbbecColorConfig(**{
                    k: v for k, v in color_data.items()
                    if k in OrbbecColorConfig.__dataclass_fields__
                })
            
            # 解析深度流配置
            if 'depth' in orbbec_data:
                depth_data = orbbec_data['depth']
                orbbec_config.depth = OrbbecDepthConfig(**{
                    k: v for k, v in depth_data.items()
                    if k in OrbbecDepthConfig.__dataclass_fields__
                })
            
            # 解析深度范围配置
            if 'depth_range' in orbbec_data:
                range_data = orbbec_data['depth_range']
                orbbec_config.depth_range = OrbbecDepthRangeConfig(**{
                    k: v for k, v in range_data.items()
                    if k in OrbbecDepthRangeConfig.__dataclass_fields__
                })
            
            # 其他 Orbbec 配置
            if 'align_mode' in orbbec_data:
                orbbec_config.align_mode = orbbec_data['align_mode']
        
        # 解析 RealSense 配置
        realsense_config = RealSenseConfig()
        if 'realsense' in cam_data:
            realsense_data = cam_data['realsense']
            realsense_config = RealSenseConfig(**{
                k: v for k, v in realsense_data.items()
                if k in RealSenseConfig.__dataclass_fields__
            })
        
        # 创建 CameraConfig
        config.camera = CameraConfig(
            enabled=cam_data.get('enabled', True),
            required=cam_data.get('required', False),
            type=cam_data.get('type', 'auto'),
            orbbec=orbbec_config,
            realsense=realsense_config,
            # 向后兼容
            width=cam_data.get('width', 1280),
            height=cam_data.get('height', 720),
            fps=cam_data.get('fps', 30),
            enable_depth=cam_data.get('enable_depth', True),
            serial_number=cam_data.get('serial_number', None)
        )
